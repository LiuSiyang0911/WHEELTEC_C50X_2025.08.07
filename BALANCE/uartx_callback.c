        ...
        if(Flag_PID==2) //Analyze the data //分析数据
        {
            ...
            else  if(Receive[1]!=0x23)
            {
                ...
                switch(Receive[1])
                {
					...
					//PID参数设置
					case 0x31:
						Set_Robot_PI_Param(Data,-1,-1);/* 使用-1代表相应的参数不设置 */
						break;
					case 0x32:
						Set_Robot_PI_Param(-1,Data,-1);/* 使用-1代表相应的参数不设置 */
						break;
//					case 0x33: Set_Robot_PI_Param(-1,-1,Data);/* 使用-1代表相应的参数不设置 */
//						break;
...
//          case 0x37: robot_control.out_limit_step = Data; break;					
            ...
            else if( Receive[1]==0x23 ) //APP上点击“发送所有数据”处理方法
            {
                ...
                if( i<=50 ) //数据在可接受范围
                {
                    ...
                    //kp、ki
                    Set_Robot_PI_Param(dataArray[1],dataArray[2],dataArray[3]);
...
//										robot_control.out_limit_step = dataArray[7];
