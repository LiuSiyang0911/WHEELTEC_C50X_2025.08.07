...
void Handle_AKM_AutoRecharge(uint8_t* temp_rxbuf)
{
	...
	if(charger.AllowRecharge==0) 
	{
		...
		if( robot.type==2 || robot.type==3 )  Set_Robot_PI_Param(300,300,0); 
		if( robot.type==4 ) Set_Robot_PI_Param(400,100,0); 
		if( robot.type==5 ) Set_Robot_PI_Param(50,200,0); 
	}
	else
	{	
		if( robot.type==0 || robot.type==1 ) leave_times=80, charger.RedRefalsh_Time=100; //原80，50
		if( robot.type==2 || robot.type==3 ) Set_Robot_PI_Param(600,600,0) , charger.RedRefalsh_Time=100; 
